...
class Initialize:
    ...
    def keyBuffer(self, e):
        ...
        try:
            e.widget.config(text=self.strJoiner)
        except View.tkinter.TclError:
    ...
    def openWindow(self, e):
        ...
        self.newWindow.wm_protocol(
            "WM_DELETE_WINDOW", lambda: self.newWindow.withdraw()
        ...
        try:
            with open(os.path.join(PATHJS), "r") as file:
                ...
                for id, data in hotkeyData.items():
                    self.treeCreador.insert(
                        "", "end", id, text=data["text"], values=data["values"]
                    )
                return hotkeyData
